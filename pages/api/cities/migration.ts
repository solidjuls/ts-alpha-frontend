...
export default async function handler(req, res) {
    try {
        const updates = [];
        const csvFilePath = path.join(__dirname, '../../../../../python/initial_data_seeding/Alpha migrating - CITIES.csv');
        
        fs.createReadStream(csvFilePath)
        .pipe(csvParser({ headers: ['ID', 'city', 'city_ascii', 'lat', 'lng', 'fed', 'province', 'Timezone'] }))
        .on('data', (row) => {
          if (row.city !== 'city') {
