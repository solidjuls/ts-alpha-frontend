...
export const usePlayersFilters = (users: UserType[] | undefined, countries: Country[] | undefined) => {
  ...
  const countryOptions = useMemo(() => 
    countries?.map(country => ({ value: country.id || '', label: country.country_name || '' })) || [], 
    [countries]
  );
  ...
  const handlePlayerChange = (selectedOptions: MultiValue<SelectOption>) => {
    const selectedValues = selectedOptions ? selectedOptions.map(option => ({ code: option.value, name: option.label })) : [];
  ...
  const hasAnySelections = playersSelected?.length > 0 || countriesSelected?.length > 0 || playdekSelected?.length > 0;
