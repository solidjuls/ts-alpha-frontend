...
const WinTypeChart: React.FC<WinTypeChartProps> = ({ playerId, fromDate }) => {
  ...
  // Calculate win rates
  const usaWinRate = winTypeStats?.USA
    ? (
        (winTypeStats.USA.wins /
          (winTypeStats.USA.wins + winTypeStats.USA.losses + winTypeStats.USA.ties)) *
        100
      ).toFixed(1)
    : "0";
  const ussrWinRate = winTypeStats?.USSR
    ? (
        (winTypeStats.USSR.wins /
  ...
  const totalUSAGames = winTypeStats?.USA
    ? winTypeStats.USA.wins + winTypeStats.USA.losses + winTypeStats.USA.ties
    : 0;
  const totalUSSRGames = winTypeStats?.USSR
    ? winTypeStats.USSR.wins + winTypeStats.USSR.losses + winTypeStats.USSR.ties
    : 0;
